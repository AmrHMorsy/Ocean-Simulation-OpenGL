...
Ocean::Ocean( glm::mat4 v, glm::mat4 proj, glm::vec3 c, glm::vec3 front, float fv, progressbar * b, glm::mat4 light_proj, glm::mat4 _light_view): num_progress_bars(40),light_position({0.0f, 10.0f, 0.0f}), light_color({100.0f, 100.0f, 100.0f}), view(v), projection(proj), camera(c), file_paths( {"../assets/Ocean/Albedo.jpeg", "../assets/Ocean/AO.jpg", "../assets/Ocean/Normal.jpg", "../assets/Ocean/Roughness.jpg", "../assets/Ocean/Displacement.jpg" } ), camera_front(front), fov(fv), number_ocean_patches(0), bar(b), light_projection(light_proj), light_view(_light_view)
{
...
void Ocean::render( unsigned int shadow_shader_program )
{
...
void Ocean::render( Shadow * shadow )
{
    glUseProgram(shader_program) ;
    update_time() ;
    activate_textures() ;
